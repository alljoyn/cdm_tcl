...
static bool gEmitSignalOnSetProperty = false;
...
AJ_Status Cdm_Init(bool emitSignalOnSetProperty)
{
    ...
    objInfoLast = NULL;
    ajObjects = NULL;
    gEmitSignalOnSetProperty = emitSignalOnSetProperty;
    ...
    if (status == AJ_OK) {
        ...
        if (strcmp(sig, variant) != 0) {
            ...
            if (objInfo && intfInfo && intfInfo->handler && intfInfo->handler->OnSetProperty) {
                uint8_t memberIndex = GetMemberIndex(propId);
                status = intfInfo->handler->OnSetProperty(busAttachment, msg, objInfo->path, memberIndex, gEmitSignalOnSetProperty);
            } else {
                status = AJ_ERR_NULL;
            }
        }
