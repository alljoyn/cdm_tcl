...
static AJ_Status TriggerSensor_OnGetProperty(AJ_BusAttachment* busAttachment, AJ_Message* replyMsg, const char* objPath, uint8_t memberIndex)
{
    ...
    switch (memberIndex) {
        ...
        case TRIGGERSENSOR_PROP_CURRENTLY_TRIGGERED:
        {
            ...
            memset(&currently_triggered, 0, sizeof(bool));
...
static AJ_Status TriggerSensor_OnSetProperty(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex, bool emitOnSet)
{
    AJ_Status status = AJ_ERR_INVALID;
