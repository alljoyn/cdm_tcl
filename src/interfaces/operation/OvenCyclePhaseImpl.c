...
static AJ_Status OvenCyclePhase_OnSetProperty(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex, bool emitOnSet)
{
    AJ_Status status = AJ_ERR_INVALID;
...
static AJ_Status OvenCyclePhase_OnMethodHandler(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex)
{
    ...
    case OVENCYCLEPHASE_METHOD_GET_VENDOR_PHASES_DESCRIPTION:
    {
        ...
        memset(&phases_description, 0, sizeof(Array_OvenCyclePhase_CyclePhaseDescriptor));
