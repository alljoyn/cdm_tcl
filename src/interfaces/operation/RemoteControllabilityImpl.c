...
static AJ_Status RemoteControllability_OnGetProperty(AJ_BusAttachment* busAttachment, AJ_Message* replyMsg, const char* objPath, uint8_t memberIndex)
{
    ...
    switch (memberIndex) {
        ...
        case REMOTECONTROLLABILITY_PROP_IS_CONTROLLABLE:
        {
            ...
            memset(&is_controllable, 0, sizeof(bool));
...
static AJ_Status RemoteControllability_OnSetProperty(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex, bool emitOnSet)
{
    AJ_Status status = AJ_ERR_INVALID;
