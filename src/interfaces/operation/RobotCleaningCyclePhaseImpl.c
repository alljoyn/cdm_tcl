...
const char* const intfDescOperationRobotCleaningCyclePhase[] = {
...
void FreeFields_RobotCleaningCyclePhase_CyclePhaseDescriptor(RobotCleaningCyclePhase_CyclePhaseDescriptor* value)
{
...
void CopyArray_RobotCleaningCyclePhase_CyclePhaseDescriptor(Array_RobotCleaningCyclePhase_CyclePhaseDescriptor* value, Array_RobotCleaningCyclePhase_CyclePhaseDescriptor* copy)
{
    if (value->elems) {
        copy->elems = malloc(value->numElems * sizeof(RobotCleaningCyclePhase_CyclePhaseDescriptor));
...
void FreeArray_RobotCleaningCyclePhase_CyclePhaseDescriptor(Array_RobotCleaningCyclePhase_CyclePhaseDescriptor* value)
{
    for (size_t i = 0; i < value->numElems; ++i) {
        FreeFields_RobotCleaningCyclePhase_CyclePhaseDescriptor(&value->elems[i]);
    }
    free(value->elems);
}
...
static AJ_Status RobotCleaningCyclePhase_GetCyclePhase(AJ_BusAttachment* busAttachment, const char* objPath, uint8_t* out)
{
    if (!objPath || !out) {
        return AJ_ERR_INVALID;
    }

    RobotCleaningCyclePhaseModel* model = (RobotCleaningCyclePhaseModel*)GetInterfaceModel(objPath, INTERFACE_NAME);
    if (!model) {
...
static AJ_Status RobotCleaningCyclePhase_GetSupportedCyclePhases(AJ_BusAttachment* busAttachment, const char* objPath, Array_uint8* out)
{
    if (!objPath || !out) {
        return AJ_ERR_INVALID;
    }
...
static AJ_Status Cdm_RobotCleaningCyclePhase_CallGetVendorPhasesDescription(AJ_BusAttachment* busAttachment, const char *objPath, char const* languageTag, Array_RobotCleaningCyclePhase_CyclePhaseDescriptor* phasesDescription)
{
    if (!objPath) {
        return AJ_ERR_INVALID;
    }
...
/*
   Handler functions
*/
static AJ_Status RobotCleaningCyclePhase_OnGetProperty(AJ_BusAttachment* busAttachment, AJ_Message* replyMsg, const char* objPath, uint8_t memberIndex)
{
    AJ_Status status = AJ_ERR_INVALID;

    switch (memberIndex) {
        case ROBOTCLEANINGCYCLEPHASE_PROP_VERSION:
            status = AJ_MarshalArgs(replyMsg, "q", INTERFACE_VERSION);
            break;

        case ROBOTCLEANINGCYCLEPHASE_PROP_CYCLE_PHASE:
        {
            uint8_t cycle_phase;
            memset(&cycle_phase, 0, sizeof(uint8_t));
            status = RobotCleaningCyclePhase_GetCyclePhase(busAttachment, objPath, &cycle_phase);
            if (status == AJ_OK) {
        ...
        case ROBOTCLEANINGCYCLEPHASE_PROP_SUPPORTED_CYCLE_PHASES:
        {
            Array_uint8 supported_cycle_phases;
            memset(&supported_cycle_phases, 0, sizeof(Array_uint8));
            status = RobotCleaningCyclePhase_GetSupportedCyclePhases(busAttachment, objPath, &supported_cycle_phases);
            if (status == AJ_OK) {
...
static AJ_Status RobotCleaningCyclePhase_OnSetProperty(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex, bool emitOnSet)
{
    AJ_Status status = AJ_ERR_INVALID;
...
static AJ_Status RobotCleaningCyclePhase_OnMethodHandler(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex)
{
    ...
    case ROBOTCLEANINGCYCLEPHASE_METHOD_GET_VENDOR_PHASES_DESCRIPTION:
    {
        AJ_Message reply;
        ...
        Array_RobotCleaningCyclePhase_CyclePhaseDescriptor phases_description;
        memset(&phases_description, 0, sizeof(Array_RobotCleaningCyclePhase_CyclePhaseDescriptor));
        ...
        FreeArray_RobotCleaningCyclePhase_CyclePhaseDescriptor(&phases_description);
        break;
    }
    }
...
const InterfaceHandler intfHandlerOperationRobotCleaningCyclePhase = {
    RobotCleaningCyclePhase_OnGetProperty,
    RobotCleaningCyclePhase_OnSetProperty,
    RobotCleaningCyclePhase_OnMethodHandler
};
