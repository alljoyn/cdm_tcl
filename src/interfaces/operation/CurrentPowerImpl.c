...
static AJ_Status CurrentPower_OnGetProperty(AJ_BusAttachment* busAttachment, AJ_Message* replyMsg, const char* objPath, uint8_t memberIndex)
{
    ...
    switch (memberIndex) {
        ...
        case CURRENTPOWER_PROP_CURRENT_POWER:
        {
            ...
            memset(&current_power, 0, sizeof(double));
...
static AJ_Status CurrentPower_OnSetProperty(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex, bool emitOnSet)
{
    AJ_Status status = AJ_ERR_INVALID;
