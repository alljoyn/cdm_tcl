...
static AJ_Status RepeatMode_SetRepeatMode(AJ_BusAttachment* busAttachment, const char* objPath, bool value)
{
    ...
    model->busAttachment = busAttachment;
    status = model->SetRepeatMode(model, objPath, value);
    return status;
}
...
static AJ_Status RepeatMode_OnGetProperty(AJ_BusAttachment* busAttachment, AJ_Message* replyMsg, const char* objPath, uint8_t memberIndex)
{
    ...
    switch (memberIndex) {
        ...
        {
            bool repeat_mode;
            memset(&repeat_mode, 0, sizeof(bool));
...
static AJ_Status RepeatMode_OnSetProperty(AJ_BusAttachment* busAttachment, AJ_Message* msg, const char* objPath, uint8_t memberIndex, bool emitOnSet)
{
    AJ_Status status = AJ_ERR_INVALID;
