...
/**
 * EnergyUsage interface model
 */
typedef struct {

    /**
     * Get CumulativeEnergy
     * @param[out] value current cumulative energy. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetCumulativeEnergy)(void *context, const char *objPath, double* value);
    ...
    AJ_Status (*GetUpdateMinTime)(void *context, const char *objPath, uint16_t* value);
     ...
     /**
      * Call ResetCumulativeEnergy
      */
    AJ_Status (*MethodResetCumulativeEnergy)(void *context, const char *objPath);

    AJ_BusAttachment* busAttachment;

} EnergyUsageModel;

#endif /* ENERGYUSAGEMODEL_H_ */