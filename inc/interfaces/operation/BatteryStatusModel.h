...
typedef struct {
    ...
    /**
     * Get IsCharging
     * @param[out] value current is charging. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetIsCharging)(void *context, const char *objPath, bool* value);

    AJ_BusAttachment* busAttachment;

} BatteryStatusModel;

#endif /* BATTERYSTATUSMODEL_H_ */