...
/**
 * CurrentPower interface model
 */
typedef struct {

    /**
     * Get CurrentPower
     * @param[out] value current current power. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetCurrentPower)(void *context, const char *objPath, double* value);
