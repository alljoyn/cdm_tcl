...
/**
 * TriggerSensor interface model
 */
typedef struct {

    /**
     * Get CurrentlyTriggered
     * @param[out] value current currently triggered. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetCurrentlyTriggered)(void *context, const char *objPath, bool* value);
