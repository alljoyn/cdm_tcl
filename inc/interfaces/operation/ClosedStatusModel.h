...
/**
 * ClosedStatus interface model
 */
typedef struct {

    /**
     * Get IsClosed
     * @param[out] value current is closed. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetIsClosed)(void *context, const char *objPath, bool* value);

    AJ_BusAttachment* busAttachment;

} ClosedStatusModel;

#endif /* CLOSEDSTATUSMODEL_H_ */