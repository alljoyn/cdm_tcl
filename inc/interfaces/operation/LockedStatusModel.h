...
/**
 * LockedStatus interface model
 */
typedef struct {

    /**
     * Get IsLocked
     * @param[out] value current is locked. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetIsLocked)(void *context, const char *objPath, bool* value);

    AJ_BusAttachment* busAttachment;

} LockedStatusModel;

#endif /* LOCKEDSTATUSMODEL_H_ */