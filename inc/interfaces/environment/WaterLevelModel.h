...
/**
 * WaterLevel interface model
 */
typedef struct {

    /**
     * Get SupplySource
     * @param[out] value current supply source. The caller will free the memory if
     *             the type is not a scalar.
     * @param[out] numValues if the type is an array, set the number of elements here.
     */
    AJ_Status (*GetSupplySource)(void *context, const char *objPath, WaterLevel_SupplySource* value);
